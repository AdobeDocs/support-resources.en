...
1. Attachments can be deleted directly from the **[!UICONTROL Case Details]** page using the delete icon shown on each attachment. This capability is not available for closed cases or for Workfront organizations.
1. Sorting is supported on all columns on the **[!UICONTROL My Cases]** page.
1. **[!UICONTROL AI Recommendations]** are available when **[!UICONTROL Admin Console]** is selected as the product during case creation.
